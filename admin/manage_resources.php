...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_resource'])) {
    $title       = $_POST['title']       ?? '';
    $description = $_POST['description'] ?? '';   // FIX 1: null-safe with ?? ''
    ...
    try {
        ...
        $stmt->execute([$title, $description ?: null, $faculty, $semester, $subject, $file_path, $link_url ?: null]);
...
<body class="flex min-h-screen">
    ...
    <main class="flex-1 p-8 pt-20 lg:pt-8">
        ...
        <?php if ($message): ?>
            <div class="bg-green-50 text-green-700 p-4 mb-6 rounded-2xl border border-green-100 flex items-center italic text-sm">
                <i class="fas fa-check-circle mr-2"></i> <?php echo htmlspecialchars($message); ?>
            </div>
        <?php endif; ?>
        <?php if ($error): ?>
            <div class="bg-red-50 text-red-700 p-4 mb-6 rounded-2xl border border-red-100 flex items-center italic text-sm">
        ...
        <div class="grid lg:grid-cols-3 gap-8">
            ...
            <div class="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                <h2 class="text-xl font-bold mb-6 flex items-center">
                    <i class="fas fa-plus-circle mr-2 text-red-600"></i> New Resource
                </h2>
                <form action="" method="POST" enctype="multipart/form-data" class="space-y-4">

                    <div>
                        <label class="text-[10px] font-black uppercase text-gray-400">Resource Title</label>
                        <input name="title" type="text" placeholder="e.g. C Programming Syllabus" required
                               class="w-full bg-gray-50 p-4 rounded-xl outline-none focus:ring-2 focus:ring-red-500 transition-all font-bold">
                    </div>

                    <!-- FIX 1: description field was missing from the form — added it here -->
                    <div>
                        <label class="text-[10px] font-black uppercase text-gray-400">Description <span class="normal-case font-normal">(optional)</span></label>
                        <textarea name="description" rows="2" placeholder="Brief description of the resource..."
                                  class="w-full bg-gray-50 p-4 rounded-xl outline-none focus:ring-2 focus:ring-red-500 transition-all font-bold resize-none text-sm"></textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <input name="subject" type="text" placeholder="e.g. Discrete Structure" required
                               class="w-full bg-gray-50 p-4 rounded-xl outline-none font-bold">
                    </div>

                    <div>
                        <label class="text-[10px] font-black uppercase text-gray-400">Optional Link (URL)</label>
                        <input name="link_url" type="url" placeholder="https://example.com/syllabus"
                               class="w-full bg-gray-50 p-4 rounded-xl outline-none font-bold">
                    </div>
                    ...
                    <div>
                        ...
                        <input name="resource_file" type="file"
                               class="w-full bg-gray-50 p-4 rounded-xl outline-none font-bold text-xs">
                    </div>

                    <button type="submit" name="add_resource"
                            class="w-full py-4 bg-red-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-red-700 transition-all shadow-xl shadow-red-100">
                        Publish Resource
                    </button>
                </form>
            </div>
                        ...
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-gray-50 transition-all">
                                    <td class="p-6">
                                        <p class="font-bold text-gray-900"><?php echo htmlspecialchars($r['title']); ?></p>
                                        <p class="text-[10px] text-gray-400 font-bold uppercase"><?php echo htmlspecialchars($r['subject']); ?></p>
                                        <!-- FIX 1: guard NULL description before printing -->
                                    ...
                                    <td class="p-6">
                                        <span class="bg-slate-100 text-slate-600 px-3 py-1 rounded-full text-[10px] font-black">
                                            <?php echo htmlspecialchars($r['faculty']); ?> • SEM <?php echo htmlspecialchars($r['semester']); ?>
                                        </span>
                                    </td>
                                    <td class="p-6 text-sm">
                                        ...
                                        <?php elseif (!empty($r['link_url'])): ?>
                                            <a href="<?php echo htmlspecialchars($r['link_url']); ?>" target="_blank"
                                    ...
                                    <td class="p-6 text-right">
                                        <button type="button" 
                                                onclick="if(confirm('Remove document?')) window.location.href='manage_resources.php?delete=<?php echo $r['id'] ?? $r['resource_id']; ?>'" 
                                ...
                                <tr>
                                    <td colspan="4" class="p-12 text-center text-gray-400 font-bold italic">
                                        No resources catalogued yet.
                                    </td>
                                </tr>
                                <?php endif; ?>
                            </tbody>
